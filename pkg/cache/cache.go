...
// isLRUCache 判断当前缓存是否为LRU类型
func (c *Cache[T]) isLRUCache(cache cacheInterface[T]) bool {
	return cache != nil && cache.getType() == TypeLRU
}

// isRedisCache 判断当前缓存是否为Redis类型
func (c *Cache[T]) isRedisCache(cache cacheInterface[T]) bool {
	return cache != nil && cache.getType() == TypeRedis
}
